...
export default function EndlessTileScreen({ onBack }) {
  ...
  const generateRow = useCallback((floorNum, themeObj, diff) => {
    const isSafeFloor = floorNum % 5 === 0;
    const mineRate = isSafeFloor ? 0 : 0.15 + (diff / 100) * 0.1;
  ...
  const initGame = () => {
    const initialRows = [];
    for (let i = 0; i < 10; i++) {
      initialRows.push(generateRow(0, THEMES.CLASSIC, 100)); // Initial rows are safe-ish
    }
    // Make sure starting row is safe
  ...
  const update = (dt) => {
    ...
    setScrollPos(prev => {
      ...
      if (next >= 1) {
        ...
        next -= 1;
        setRows(currentRows => {
          const newRows = [...currentRows.slice(1), generateRow(floor + 10, theme, difficulty)];
          return newRows;
        });
        setChicken(prevC => ({ ...prevC, r: prevC.r - 1 }));
        setFloor(prevF => {
            const nextF = prevF + 1;
            // Theme rotation every 5 floors
            if (nextF % 5 === 1 && nextF > 1) {
                const nextTheme = THEME_LIST[Math.floor(Math.random() * THEME_LIST.length)];
                setTheme(nextTheme);
            }
            // Difficulty scaling every 25 floors
            if (nextF % 25 === 0) {
                const inc = 5 * (nextF / 25);
                setDifficulty(d => d + inc);
                setScrollSpeed(s => s * (1 + inc / 100));
            }
            return nextF;
        });
      }
    ...
    // Death check: if chicken falls off bottom row
    if (chicken.r < 0) {
      triggerGameOver();
      return;
    }
        ...
        if (targetRow) {
            ...
            if (tile.isMine) {
                ...
                if (tile.state === 'hidden') {
                    ...
                    // audio.walk(); // Basic walk if available
                }
    ...
    <div className={`endless-screen theme-${theme.id}`} style={{ backgroundColor: theme.color }}>
      ...
      <div className="endless-controls">
        {isSafeFloor && !isGameOver && (
            <button className="btn-pause" onClick={() => { setIsPaused(!isPaused); audio.init(); }}>
